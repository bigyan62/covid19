<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title>new-website</title>
    <link rel="shortcut icon" href="images/favicon.ico" type="image/x-icon">
    <link rel="icon" href="images/favicon.ico" type="image/x-icon">
    <link rel="stylesheet" type="text/css" href="css/bootstrap.css">
    <link rel="stylesheet" type="text/css" href="css/all.css">
    <link rel="stylesheet" type="text/css" href="styles.css">
    <link rel="stylesheet" type="text/css" href="css/responsive.css">
</head>
<body class="dark-blue-bg">
<header class="site-header light-blue-bg">


    <div class="container">
        <div class="row align-items-center">

            <div class="col-5 col-md-3">
                <div class="logo">
                    <a href="index.php" title="logo">
                        <img src="images/covid-logo.png" alt="Logo">
                    </a>
                </div>
            </div>

            <div class="col-7 col-md-9">
                <div class="hamburger-menu">
                    <span class="bar">
                      <svg height="384pt" viewBox="0 -53 384 384" width="384pt" xmlns="http://www.w3.org/2000/svg"><path
                                  d="m368 154.667969h-352c-8.832031 0-16-7.167969-16-16s7.167969-16 16-16h352c8.832031 0 16 7.167969 16 16s-7.167969 16-16 16zm0 0"/><path
                                  d="m368 32h-352c-8.832031 0-16-7.167969-16-16s7.167969-16 16-16h352c8.832031 0 16 7.167969 16 16s-7.167969 16-16 16zm0 0"/><path
                                  d="m368 277.332031h-352c-8.832031 0-16-7.167969-16-16s7.167969-16 16-16h352c8.832031 0 16 7.167969 16 16s-7.167969 16-16 16zm0 0"/></svg>
                    </span>
                </div>
                <div class="overlay"></div>
                <nav class="nav justify-content-end">
                    <span class="menu-close">
                        <i class="fas fa-times"></i>
                    </span>
                    <div class="menu-list">

                        <ul class="primary-menu d-flex flex-wrap justify-content-end">
                            <li><a href="#" title="">COVID-19</a></li>
                            <li><a href="#" title="">Spread</a></li>
                            <li><a href="#" title="">Symptoms</a></li>
                            <li><a href="#" title="">Prevention</a></li>
                            <li><a href="#" title="">News</a></li>
                        </ul>
                    </div>
                </nav>
            </div>
        </div>
    </div>
</header>
